...
public class AlgoSFUPMinerUemax {
	...
	public void runAlgorithm(String input, String output) throws IOException {
		...
		Map<Integer, ExtentUtilityList> mapItemToUtilityList = new HashMap<Integer, ExtentUtilityList>();
		...
		try {
			...
			while ((thisLine = myInput.readLine()) != null) {
				...
				for(Pair pair : revisedTransaction){
					...
					ExtentUtilityList utilityListOfItem = mapItemToUtilityList.get(pair.item);
					
					// Add a new Element to the utility list of this item corresponding to this transaction
					ExtentElement element = new ExtentElement(tid, pair.utility, remainingUtility);
	 ...
	 * @throws IOException
	 */
	private void SFUPMiner(int [] prefix, ExtentUtilityList pUL, List<ExtentUtilityList> ULs, SkylineList psfupList[], List<Skyline> skylineList, int [] uEmax)
			throws IOException {
	...
	private ExtentUtilityList construct(ExtentUtilityList P, ExtentUtilityList px, ExtentUtilityList py) {
		...
		for(ExtentElement ex : px.elements){
			...
			if(P == null){
				...
				ExtentElement eXY = new ExtentElement(ex.tid, ex.itemSetutils + ey.itemSetutils, ey.rutils);
				// add the new element to the utility list of pXY
				pxyUL.addElement(eXY);
				
			}else{
				// find the element in the utility list of p wih the same tid
				ExtentElement e = findElementWithTID(P, ex.tid);
				if(e != null){
					// Create new element
					ExtentElement eXY = new ExtentElement(ex.tid, ex.itemSetutils + ey.itemSetutils - e.itemSetutils,
								ey.rutils);
					// add the new element to the utility list of pXY
					pxyUL.addElement(eXY);
				}
...
//		buffer.append(System.lineSeparator());
	...
	private void checkMemory() {
		...
		double currentMemory = (gfg.totalMemory() -  gfg.freeMemory())
				/ 1024d / 1024d;
	...
	public void printStats() {
		...
		System.out.println(" Search itemsets count : " + searchCount);
                System.out.println(" The number of joining two utility lists: " + numberOfJoins);
